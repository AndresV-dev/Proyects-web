...
export const supabaseAdmin = createClient<Database>(process.env.NEXT_PUBLIC_SUPABASE_URL || '', process.env.SUPABASE_SERVICE_ROLE || '');

const upsertProductoRecord = async (product: Stripe.Product) => {
    ...
    const { error } = await supabaseAdmin.from('products').upsert([productData]).single()
    if(error){
        throw error;
    }
...
const upsertPriceRecord = async (price: Stripe.Price) => {
    ...
    const { error } = await supabaseAdmin.from('prices').upsert([priceData]).single()
...
const createOrRetrieveACostumer = async ({email, uuid}: {email: string, uuid: string}) => {
...
const manageSubscriptionStatusChange = async (subscriptionId: string, customerId: string, createAction = false) => {
    ...
    console.log(`Subscription Updated: ${subscriptionId} for ${uuid}`);
...
export {
    upsertProductoRecord,
    upsertPriceRecord,
    createOrRetrieveACostumer,
